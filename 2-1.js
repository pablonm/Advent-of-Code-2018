...
const ocurrencesOf = (string, char) => {
...
let hasThree
let inputsWithTwo = 0
let inputsWithThree = 0
for (let i = 0; i < input.length; i++) {

    hasTwo = false;
    hasThree = false;
    for (let j = 0; j < input[i].length; j++) {
        let occ = ocurrencesOf(input[i], input[i][j])
        if (occ === 2) hasTwo = true
        if (occ === 3) hasThree = true
    }
    if (hasTwo) inputsWithTwo++
    if (hasThree) inputsWithThree++
}
console.log(inputsWithTwo * inputsWithThree)