...
getDate = entry => {
    return new Date(entry.split('[')[1].split(']')[0])
}

minutesDiff = (entry1, entry2) => {
    return Math.abs(differenceInMinutes(getDate(sortedInput[0]), getDate(sortedInput[1])))
}

fixShiftStartDate = entry => {
    if (getHours(getDate(entry)) !== 0) {
        let newDate = addDays(getDate(entry), 1)
        newDate = setHours(newDate, 0)
        newDate = setMinutes(newDate, 0)
        return '[' + format(newDate, 'YYYY-MM-DD HH:mm') + ']' + entry.split(']')[1]
    }
    return entry
}
...
const processedInput = _
    .orderBy(input, entry => getDate(entry))
    .map(entry => fixShiftStartDate(entry))
...
for(let i = 0; i < processedInput.length; i++) {
    const entryType = processedInput[i].split('] ')[1].split(' ')[0]
    switch (entryType) {
        case 'Guard':
            currentGuard = getGuardID(processedInput[i])
            if(!hoursByGuard.find(g => g.id === getGuardID(processedInput[i]))) {
                hoursByGuard.push({
                    id: getGuardID(processedInput[i]),
                    totalSleep: 0,
                    sleepByHour: []
                })
            }
            break
        case 'falls':
            sleepMinute = getMinutes(getDate(processedInput[i]))
            break
        case 'wakes':
            wakeMinute = getMinutes(getDate(processedInput[i]))
            hoursByGuard.map(g => {
                if (g.id === currentGuard) {
                    let sleepedMinutes = wakeMinute - sleepMinute
                    g.totalSleep += sleepedMinutes
                    for (let i = sleepMinute; i < sleepMinute + sleepedMinutes; i++) {
                        if (!g.sleepByHour[i]) g.sleepByHour[i] = 1
                        else g.sleepByHour[i]++
                    }
                }
                return g
            })
...
hoursByGuard = hoursByGuard.map(g => {
    return {
        id: g.id,
        maxSleepMinuteTime: _.maxBy(g.sleepByHour, h => h),
        maxSleepMinute: g.sleepByHour.indexOf(_.maxBy(g.sleepByHour, h => h))
    }
})

hoursByGuardMax = _.maxBy(hoursByGuard, g => g.maxSleepMinuteTime)


console.log(hoursByGuardMax.id * hoursByGuardMax.maxSleepMinute)
